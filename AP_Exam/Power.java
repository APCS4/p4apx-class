...
public class Power extends Question {
	/**
	 * Author: Alex Clanton, Mithil Pujar Power Button, rando questions and answers
	 *
	 *
	 * Ver 1.0
	 */
	...
	protected void setupQuestionData() {
		...
		case 0:// q1
			...
			pick = loadArray("Do While", "Switch Case", "For", "If,Else,For", "Thats done by the user");// array for
																										// stroing
																										// answers
		...
		case 1:// q2
			...
			pick = loadArray("For:", "for(int z=12; z<=103; z++);", "for{in i=1: i<6:}", "for {int i = 1: i<=6:} ()",
					"for: a=1, b=2, i++");
		...
		case 2:// q3
			...
			pick = loadArray("loops such as for loops", "Recursion", "Instance Initialization", "Public Static Void",
					"if else statements");
		...
		case 3:// q4
			...
			pick = loadArray("to replace a for loopo in all cases for shortening code", "Recursion only",
					"Executing a block of code at least once and then repeatedly executing it based on a boolean condition",
					"Allowing code to be repeatedly executed",
					"To create simultaneous control flows for simplified code");
		 ...
		 * "\nfor(intp=4;p<=100;p=p+4)\n System.out.println(p);"); this.choiceA =
		 * pick[0]; this.choiceB = pick[1]; this.choiceC = pick[2]; this.choiceD =
		 * pick[3]; this.choiceE = pick[4]; this.answerKey = Ans(pick,
		 * "\nfor(intp=4;p<=100;p=p+4)\n System.out.println(p);"); this.answer =
	...
	private String[] loadArray(String ques1, String ques2, String ques3, String ques4, String ques5)// all choices
																									// passed as an
																									// argument
	{
		/**
		 * loadarray is used to take the 4 answer choices and randomize them in an
		 * array, then return the choice
		 *
		 *
		 *
		 */
	...
	/**
	 * answer choice check to find if they are true then return that value as an
	 * char
	 * 
	 * @param ques
	 * @param an
